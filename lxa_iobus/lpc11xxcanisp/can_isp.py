...
class CanIsp:
    ...
    def write(self, filename, section):
        ...
        if len(data) > length:
            self.console_log("Supplied Image is too long for section. Allowed {} bytes, is {} bytes".format(length, len(data)))
            exit(1)
        ...
        stop_t = time.time()
        self.console_log("Write", len(data), "in", stop_t-start_t, ":", len(data)/(stop_t-start_t),"Bytes/sec")
    ...
    def read(self, filename, section):
        ...
        stop_t = time.time()
        self.console_log("Read", length, "in", stop_t-start_t, ":", length/(stop_t-start_t),"Bytes/sec")
